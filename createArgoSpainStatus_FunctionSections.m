function createArgoSpainStatus_FunctionSections(OneFloatData,Limits);
...
if isfield(OneFloatData,'oxys')
    if ~isempty(OneFloatData.oxys)
        if size(OneFloatData.oxys,2)>1 && sum(isnan(OneFloatData.oxys(:))) ~= length(OneFloatData.oxys(:))
            RepresentaOxigen=0;
...
[Sjulds,IA,IC] = unique(OneFloatData.julds);
if length(Sjulds)~=length(OneFloatData.julds) || issorted(OneFloatData.julds)==0
    OneFloatData.tems=OneFloatData.tems(:,IA);
    OneFloatData.pres=OneFloatData.pres(:,IA);
    OneFloatData.sals=OneFloatData.sals(:,IA);
    OneFloatData.julds=Sjulds;
    if RepresentaOxigen==1
        OneFloatData.oxys=OneFloatData.oxys(:,IA);
    end
end

color=linspace(1,64,length(OneFloatData.julds));
cl=parula;
if size(OneFloatData.julds,2)>5
    LV=ceil(linspace(1,size(OneFloatData.julds,2),5));
end

presiar=[1:10:Limits.maxP]';

%% Temperature
if size(OneFloatData.tems,2)>1 && sum(isnan(OneFloatData.tems(:))) ~= length(OneFloatData.tems(:))
    rangoG=nice([Limits.minT  Limits.maxT],2);
    if isnan(sum(rangoG))==1
        rangoG=rangoT;
    end
    for inp=1:size(OneFloatData.tems,2)
        ptem=OneFloatData.tems(:,inp);
        ppre=OneFloatData.pres(:,inp);
    ...
    [~,ch]=contourf(OneFloatData.julds,-presiar(1:Locate(presiar,-Limits.UpperPL(1))+1),temsAic,linspace(Limits.minT,Limits.maxT,40));set(ch,'edgecolor','none');hold on
    contour(OneFloatData.julds,-presiar(1:Locate(presiar,-Limits.UpperPL(1))+1),temsAic,rangoG(1:2:end),'color',[0.6 0.6 0.6],'linewidth',0.15);
    [cs,h]=contour(OneFloatData.julds,-presiar(1:Locate(presiar,-Limits.UpperPL(1))+1),temsAic,rangoG(1:4:end),'color',[0.6 0.6 0.6],'linewidth',0.15);
    ...
    [~,ch]=contourf(OneFloatData.julds,-presiar(Locate(presiar,-Limits.UpperPL(1)):end),temsAic,linspace(Limits.minT,Limits.maxT,40));set(ch,'edgecolor','none');hold on
    contour(OneFloatData.julds,-presiar(Locate(presiar,-Limits.UpperPL(1)):end),temsAic,rangoG(1:2:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    ...
    hT1.XMinorTick='on';
    hT1.XAxis.MinorTickValues=OneFloatData.julds;
    ...
    hT2.XMinorTick='on';
    hT2.XAxis.MinorTickValues=OneFloatData.julds;
end

%% Salinity
if size(OneFloatData.sals,2)>1 && sum(isnan(OneFloatData.sals(:))) ~= length(OneFloatData.sals(:))
    rangoG=nice([Limits.minS Limits.maxS],2);
    if isnan(sum(rangoG))==1
        rangoG=rangoS;
    end
    for inp=1:size(OneFloatData.tems,2)
        psal=OneFloatData.sals(:,inp);
        ppre=OneFloatData.pres(:,inp);
    ...
    [~,ch]=contourf(OneFloatData.julds,-presiar(1:Locate(presiar,-Limits.UpperPL(1))+1),salsAic,linspace(Limits.minS,Limits.maxS,40));set(ch,'edgecolor','none');hold on
    contour(OneFloatData.julds,-presiar(1:Locate(presiar,-Limits.UpperPL(1))+1),salsAic,rangoG(1:2:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    [cs,h]=contour(OneFloatData.julds,-presiar(1:Locate(presiar,-Limits.UpperPL(1))+1),salsAic,rangoG(1:4:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    axis([Limits.minJ Limits.maxJ Limits.UpperPL])
    ...
    [~,ch]=contourf(OneFloatData.julds,-presiar(Locate(presiar,-Limits.UpperPL(1)):end),salsAic,linspace(Limits.minS,Limits.maxS,40));set(ch,'edgecolor','none');hold on
    contour(OneFloatData.julds,-presiar(Locate(presiar,-Limits.UpperPL(1)):end),salsAic,rangoG(1:2:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    ...
    hS1.XMinorTick='on';
    hS1.XAxis.MinorTickValues=OneFloatData.julds;
    ...
    hS2.XMinorTick='on';
    hS2.XAxis.MinorTickValues=OneFloatData.julds;
...
if RepresentaOxigen==1
    ...
    for inp=1:size(OneFloatData.oxys,2)
        poxy=OneFloatData.oxys(:,inp);
        ppre=OneFloatData.pres(:,inp);
        poxy(poxy>maxO)=NaN;
        poxy(poxy<minO)=NaN;
    ...
    [~,ch]=contourf(OneFloatData.julds,-presiar,oxysAi,linspace(Limits.minO,Limits.maxO,40));set(ch,'edgecolor','none');hold on
    contour(OneFloatData.julds,-presiar,oxysAi,rangoG(1:2:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    [cs,h]=contour(OneFloatData.julds,-presiar,oxysAi,rangoG(1:4:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    ...
    [~,ch]=contourf(OneFloatData.julds,-presiar,oxysAi,linspace(minO,maxO,40));set(ch,'edgecolor','none');hold on
    contour(OneFloatData.julds,-presiar,oxysAi,rangoG(1:2:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    [cs,h]=contour(OneFloatData.julds,-presiar,oxysAi,rangoG(1:1:end),'color',[0.6 0.6 0.6],'linewidth',0.15);hold on
    %cl=clabel(cs,'fontsize',6,'color','k','rotation',0,'BackgroundColor','w','margin',1);
    axis([Limits.minJ Limits.maxJ Limits.DeepPL])
    datetick('x',12,'keepLimits')
    colormap(jet)
