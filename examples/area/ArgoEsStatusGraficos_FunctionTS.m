function ArgoEsStatusGraficos_FunctionTS(OneFloatData,GlobalDS,Limits,hPosition);
%Colors
cl=parula;
...
%Fist profile
plot(OneFloatData.salsA(:,1),OneFloatData.ptemsA(:,1),'color',cl(ceil(color(1)),:),'linewidth',1.25);hold on
%Following profiles
for j=2:size(OneFloatData.salsA,2)
    plot(OneFloatData.salsA(:,j),OneFloatData.ptemsA(:,j),'color',cl(ceil(color(j)),:),'linewidth',1.25);
end
%Last profiles
plot(OneFloatData.salsA(:,end),OneFloatData.ptemsA(:,end),'color',cl(ceil(color(end)),:),'linewidth',3);
plot(OneFloatData.salsA(:,end),OneFloatData.ptemsA(:,end),'color','k','linewidth',1);

colormap(cl)
if size(OneFloatData.salsA,2)==1
    caxis([1 2])
else
    caxis([1 size(OneFloatData.salsA,2)])
end
